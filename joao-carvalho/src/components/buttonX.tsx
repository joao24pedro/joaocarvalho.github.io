...
interface ButtonXProps {
  ...
  link?: string;
  isLink?: boolean;
  onClick?: () => void; // Optional onClick handler for when isLink is false
  animation?: boolean;
}

const ButtonX: React.FC<ButtonXProps> = ({ text, link, isLink, onClick, animation = true }) => {
  const content = (
    <button className="button-with-icon" onClick={onClick}>
      <span style={{ fontSize: 14, textDecoration: 'none', fontWeight: 600}}>{text}</span>
      <div className="icon">
      {animation && (
        <span>
